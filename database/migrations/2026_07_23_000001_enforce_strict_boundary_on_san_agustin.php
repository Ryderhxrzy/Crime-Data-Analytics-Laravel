...
/**
 * Cleanup after 000009 ran with a 40 m boundary tolerance: some incidents were
 * placed just OUTSIDE the official PSGC barangay polygon, and some belonged to
 * streets that a stricter re-clip has since removed from the street geojson
 * (e.g. Emma Street, which lies entirely outside the barangay).
 *
 * Order of operations:
 *   1. DELETE incidents strictly outside the PSGC polygon (no tolerance).
 *   2. DELETE incidents whose street no longer exists in the clipped geojson,
 *      so the data always matches what the map can draw.
 *   3. TOP UP streets left with zero incidents (2-3 rows, SAU-2026-#### codes,
 *      strictly-inside placement).
 *   4. TRIM back to TARGET_TOTAL via the newest-first per-street round-robin.
 *
 * Idempotent: a re-run finds nothing outside, no orphan streets, no empty
 * streets, nothing above the cap.
 */
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (empty($streets) || empty($this->boundary)) {
            return;
        }

        $this->deleteOutsideAndOrphaned($streets);
    ...
    {
        if (Schema::hasTable(self::TABLE)) {
            DB::table(self::TABLE)->where('incident_code', 'like', 'SAU-%')->delete();
        }
    }

    // ---------------------------------------------------------------- cleanup

    private function deleteOutsideAndOrphaned(array $streets): void
    {
        $rows = DB::table(self::TABLE)->get(['id', 'latitude', 'longitude', 'address_details']);

        $bad = $rows->filter(function ($r) use ($streets) {
            if (!$this->contains((float) $r->longitude, (float) $r->latitude)) {
                return true;   // strictly outside the barangay polygon
            }

            $street = trim(explode(',', (string) $r->address_details)[0] ?? '');

            return $street === '' || !isset($streets[$street]);   // street gone from the map
        })->pluck('id');

        foreach ($bad->chunk(500) as $chunk) {
    ...
    private function topUpEmptyStreets(array $streets): void
    {
        ...
        foreach ($streets as $name => $street) {
            ...
            $seed = crc32('san-agustin-strict/' . $name);
            $target = 2 + $this->nextInt($seed, 2);

            for ($k = 0; $k < $target; $k++) {
                $sequence++;
                $code = sprintf('SAU-2026-%04d', $sequence);
    ...
    /** Point along the street, nudged sideways, retried until STRICTLY inside */
    private function pointAlongStreet(array $street, int &$seed): array
    {
        ...
        for ($try = 0; $try < 12; $try++) {
            ...
            if ($this->contains($plng, $plat)) {
                return [$plng, $plat];
            }
        }

        // Last resort: a point on the clipped street line itself is inside
        return [$lng, $lat];
    }
